...
const _private = require('./helpers/private');
var Gruntifier = function (grunt, target, done, bust) {

	// Config object
	var config = grunt.config("modernizr")[target],
		_defaults = require("./helpers/defaults"),
		_private = require("./helpers/private"),
	...
	const _privateCoreMap = {};
	Object.keys(_private.core).forEach(function(key){
		_privateCoreMap[key] = true;
	});
	...
	_Gruntifier.prototype = {
		...
		getRequests : function (tests) {
			return tests.filter(function (dep) {
				return (!!_privateCoreMap[dep]) || this.isExtra(dep);
			}.bind(this));
		},
		...
		makeRequests : function (tests) {
			...
			Promise.all([
				...
				if (!_quiet && communityRequests.length) {
					throw new Error(`Unsupported modernizr tests have been requested. These tests cannot be used: [${communityRequests.map(function(s){return `"${s}"`;}).join(", ")}]`);
				}
				...
				this.finalize(main + custom, tests, customTests);
			}.bind(this));
		},

		addCSSPrefix : function (build) {
			var prefixClass = config.extensibility.cssclassprefix;
		...
		addLicensePrefix : function (build, tests, customTests) {
			build = ";" + build + ";";
		...
		finalize : function (data, tests, customTests) {
			...
			if (config.uglify) {
				...
				const minifiedResult = uglify.minify(build);
				build = minifiedResult.code;
			} else if (!_quiet) {
				grunt.log.ok("Skipping uglify");
			}

			// Add CSS Prefix (if necessary)
			if (typeof (config.extensibility || {}).cssclassprefix === "string") {
				build = this.addCSSPrefix(build);
				tests[tests.indexOf('cssclassprefix')] = 'cssclassprefix:' + config.extensibility.cssclassprefix.replace(/-/g, '!');
			}

			// Prefix with Modernizr licence
			build = this.addLicensePrefix(build, tests, customTests);
			...
			if (typeof config.handler === 'function') {
				if (config.handler.length === 2) {
					return config.handler(tests, done);
				} else {
